...
function obtenerTipoGestion(){
    const requestOptions = {
      ...
      fetch("http://144.126.210.74:8080/api/tipo_gestion/?_size=300", requestOptions)
        .then((response) => response.json())
        .then((json) => {
            json.forEach(completarSelectTipoGestion);
...
function completarEtiquetaEliminar(element,index,arr){
    ...
    etiquetaEliminar.innerHTML = "<p>¿Desea eliminar esta gestión? ID: " + id_gestion + " de fecha "+ fecha_gestion+"</p>";
...
var fechaActual = new Date(fecha_registro);
var fechaFormateada = fechaActual.toLocaleString('es-ES',{
    hour12 :false,
    year:'numeric',
    month:'2-digit',
